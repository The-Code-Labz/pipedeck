...
# --- PipeWire (warn only — server degrades gracefully without it) -------------
if command -v pactl >/dev/null 2>&1 && pactl info >/dev/null 2>&1; then
  log "PipeWire detected: $(pactl info | awk -F': ' '/^Server Name/{print $2}')"
else
  warn "PipeWire not detected — installing audio stack"
  pkg_install $PKGS_AUDIO || warn "could not install PipeWire automatically; install it manually and log back in"
  warn "after installing PipeWire, log out/in so your session picks it up"
fi
